...
Python_Learning_Assistant/
  ...
  content/
    manifest.json         Topic order and schema metadata (26 topics)
...
Learner code is compiled with the filename `<your code>` and module-level failures are reported cleanly: a `SyntaxError` becomes "Your code has a syntax error on line N: …", and other top-level exceptions print only the learner's own traceback frames — never the runner's wrapper frames (`<string>`, `exec(...)`). Do not revert to a bare `traceback.print_exc()`, which leaked the wrapper script's internals to beginners. When the top-level run fails, every per-test row's `actual` field shows `"could not run - see error above"` instead of the raw exception repr (e.g. `NameError(...)` from calling a function that was never defined) — a beginner doesn't need to see the same crash restated three times in constructor syntax. Tests: `tests/test_runner.py`.
...
- Accessibility invariants (verified with axe-core; tests in `tests/test_accessibility.py`): scrollable `<pre>` code blocks (lesson, chat, overview `#syntax`/`#example`) carry `tabindex="0"` so keyboard users can scroll them; the CodeMirror editor sets `EditorView.contentAttributes` `aria-label="Code editor"` for an accessible name; the visualizer and code-popup modals move focus inside on open (`vizClose` / `codePopupEditor`) and return it to the trigger on close (`_vizReturnFocus` / `_codePopupReturnFocus`), matching the Ask AI panel's focus handling; `.lesson-code-note` uses a light slate (`#94a3b8`) over the dark code block, not a dark muted color (which gave ~1:1 contrast). Keep WCAG AA contrast (≥4.5:1) when changing muted text colors.
- `.ai-settings-panel` is `position: fixed; z-index: 800; max-height: calc(100vh - 24px); overflow-y: auto`. JS opens it to the right of the **sidebar container's own `getBoundingClientRect()`** (`sidebarRight + 8`) on wide viewports, reads `offsetHeight` after render, then clamps `top` so Save & Apply is never off-screen. Do not measure from the trigger button's rect instead — the button doesn't span the sidebar's full width, so `button.getBoundingClientRect().right + 8` can land inside the sidebar's actual right edge at wide viewports (confirmed at 2560×1440: button right 275px vs. sidebar right 300px). On narrow viewports it opens above the button when there is room, otherwise below, and clamps both `top` and `left` so the panel stays within the viewport on all screen sizes. Do not revert to `width: rect.width` (the button width) — that was the original bug that caused the panel to overlap the sidebar topic list. Do not revert to `bottom`-based positioning for the narrow fallback — that caused the panel to go above the viewport on mobile.
...
- Floating Ask AI close only hides the messenger; `New chat` starts a new parallel session — like a separate messenger thread — without erasing the current one. It must never just reset `askAiMessages` in place; that destroys the learner's prior conversation with no way back, which is the exact bug this design fixes.
...
- **Endpoint pinning (P0 — do not weaken):** for hosted providers with a fixed host (openai, anthropic, google, grok, groq), `ai_coach._pinned_endpoint(provider, endpoint, default)` is used everywhere the request URL is built — a caller-supplied `endpoint` is only honored if its hostname matches that provider's real API host (`_PROVIDER_ALLOWED_HOSTS`), otherwise it is silently ignored in favor of the real default. Azure AI Foundry has no single fixed host (every deployment is a unique per-tenant URL), so it uses `_require_azure_host()` instead, which rejects (rather than silently substitutes) any endpoint whose host isn't a recognized Azure AI/OpenAI domain suffix. Without this, a request's `endpoint` field was sent straight through as the URL the **server** connects to, with the server's own resolved API key (the `.env`-stored one) attached — letting a caller redirect the server to an attacker-chosen host and exfiltrate the stored secret. Confirmed live during a security audit. Local providers (ollama, lmstudio) are intentionally not pinned — they default to localhost and don't carry a real server-stored secret the same way. Tests: `tests/test_ai_models.py::TestEndpointPinning`.
...
- AI Coach and Ask AI streams use generation counters (`coachStreamGen`, `askAiStreamGen`) to prevent a late chunk from a previous request landing in the wrong conversation. `selectExercise()`, `startNewAskAiChat()`, `switchAskAiSession()`, and `closeAskAiSession()` (when closing the active session) all increment the counter for the surface/session they leave; `askLabCoach()`/`askFloatingAi()` capture the counter at call start (`const generation = ++coachStreamGen` / `++askAiStreamGen`) and bail out of `onChunk`, the post-stream success path, and the `catch` block if the counter has since changed. Do not remove these guards or write streamed text into `coachMessages`/`askAiMessages` without checking `generation` first — `askAiMessages` is reassigned whenever the active Ask AI session changes, and an unguarded async callback can append a stray message into the wrong session's array. A reply still streaming in when the learner switches sessions is dropped rather than delivered into the background session — an accepted v1 limitation, not a bug to fix by relaxing the guard.
- AI is optional and not configured out of the box (the default provider is local Ollama, which a first-time learner usually has not installed). The unconfigured paths must stay actionable, not dead ends: `ai_coach.NO_LOCAL_MODEL_HINT` and the local "connection refused" branch of `friendly_provider_error` both point the learner to ⚙ AI Settings and the no-install hosted route (paste an API key). `friendly_provider_error` also has a 401/expired-or-invalid-key branch (matches `"401"`, `expired_api_key`, `invalid_api_key`, or `invalid api key`) so a raw upstream error body is never shown verbatim — do not let it fall through to `str(exc)` for auth failures. `maybeShowAiOnboarding()` shows one dismissible first-run banner (`.ai-onboard-nudge`, reusing `.review-nudge` styling) when `isAiConfigured()` is false, gated by the `pySkillLabAiOnboarded` localStorage flag so it never nags. A request payload that omits `"provider"` resolves through `_default_provider()` (the server's saved `PY_SKILL_LAB_AI_PROVIDER`, falling back to `"ollama"` only when nothing is configured) rather than hardcoding `"ollama"` — do not reintroduce `payload.get("provider", "ollama")` directly. Tests: `tests/test_ai_onboarding.py`, `tests/test_ai_models.py`.
...
- Learner-facing short text that contains backticked code references — lab prompts, hints, practice question text/options/explanations — must render through `_inlineMarkdown()` (innerHTML), not `textContent`, or backticks show literally. Content authors use backticks heavily (115/122 prompts), so this matters. `_inlineMarkdown` escapes HTML first, so trusted content stays safe. Test: `tests/test_chat_rendering.py::test_lab_and_practice_text_renders_inline_code_not_literal_backticks`.
- The floating Ask AI panel has minimize/maximize/close window controls (`#askAiMinimize`, `#askAiMaximize`, `#askAiClose`). Minimize and close both collapse to the launcher bubble keeping the conversation; `toggleAskAiMaximize()` toggles `.ask-ai-panel.maximized` (wider/taller for long answers). Ask AI supports parallel chat sessions, like separate messenger threads: `askAiSessions` holds `{id, title, messages}` per chat, `askAiMessages` always points at the active session's array, and `activeAskAiSessionId` tracks which one is shown. `startNewAskAiChat()` archives the current session via `_archiveActiveAskAiSession()` (titles it from its first user message) and pushes a new session rather than overwriting the existing one, then resets the visible transcript to the welcome message with a clear fresh-start signal ("New chat started", scroll to top) — keep the `askAiMessages = [{ role: "assistant", text: ASK_AI_WELCOME_MESSAGE }]` line intact for the stream-generation guard. `switchAskAiSession(id)` moves between chats; `closeAskAiSession(id)` removes one but always keeps at least one chat open. The session tab strip (`#askAiSessionTabs`) only renders once 2+ sessions exist, so a single chat stays uncluttered. Tests: `tests/test_chat_rendering.py`.
