...
Python_Learning_Assistant/
  ...
  content/
    manifest.json         Topic order and schema metadata (19 topics)
...
Run `python -B -m pytest tests -q -p no:cacheprovider` before reporting any task complete. The current baseline is 988 tests passing. Do not ship a regression.
